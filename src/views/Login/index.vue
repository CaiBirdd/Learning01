...
//12056258288 hm#qd@23! scobys4865@sandbox.com
//获取form表单 做以上内容的统一验证 这相当于js的获取dom了
const router = useRouter()
const formRef = ref(null)
const UserStore = useUserStore() //仓库你不获取咋用啊 这一步是 获取 store 实例，而不是 实例化对象。
const loginCheck = () => {
  formRef.value.validate( async (valid) => {
    console.log(valid)
    if(valid){
      //登录
      //原来是const res = await loginAPI(form.value)
