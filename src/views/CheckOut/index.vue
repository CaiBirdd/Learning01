...
const createOrder = async () => {
  ...
  //订单提交以后 账号的购物车中的内容就被清空了 所以需要更新一下购物车
  ...
  <el-dialog v-model="showDialog" title="切换收货地址" width="30%" center>
    <div class="addressWrapper">
      <div
        ...
        :class="{ active: item.id === activeAddress.id }" @click="switchAddress(item)"
      > <!-- 这里可以立即执行 不用外面套一层箭头函数了-->
        <ul>
...
.addressWrapper {
  max-height: 500px;
  overflow-y: auto; //当元素内容在垂直方向的高度超过元素本身的高度时，会自动显示垂直滚动条，允许用户通过滚动查看溢出的内容。
}
...
.text {
  ...
  &.item {
    ...
    &:hover {
      border-color: $xtxColor;
      background: color.scale($xtxColor, $lightness: 50%); //增加亮度
    }
